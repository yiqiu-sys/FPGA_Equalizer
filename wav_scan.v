//////////////////////////////////////////////////////////////////////////////////
//                                                                              //
//                                                                              //
//  Author: meisq                                                               //
//          msq@qq.com                                                          //
//          ALINX(shanghai) Technology Co.,Ltd                                  //
//          heijin                                                              //
//     WEB: http://www.alinx.cn/                                                //
//     BBS: http://www.heijin.org/                                              //
//                                                                              //
//////////////////////////////////////////////////////////////////////////////////
//                                                                              //
// Copyright (c) 2017,ALINX(shanghai) Technology Co.,Ltd                        //
//                    All rights reserved                                       //
//                                                                              //
// This source file may be used and distributed without restriction provided    //
// that this copyright statement is not removed from the file and that any      //
// derivative work contains the original copyright notice and the associated    //
// disclaimer.                                                                  //
//                                                                              //
//////////////////////////////////////////////////////////////////////////////////

//==========================================================================
//  Revision History:
//  Date          By            Revision    Change Description
//--------------------------------------------------------------------------
//  2017/7/14    meisq         1.0         Original
//*************************************************************************/
module wav_scan(
	input                   clk,
	input                   rst,
	input                   find,
	input                   sd_init_done,             //SD card initialization completed
	output reg[3:0]         state_code,              //state indication coding,
	                                                 // 0:SD card is initializing,
				                                     // 1:wait for the button to press
				                                     // 2:looking for the WAV file
				                                     // 3:playing
	output reg              sd_sec_read,                 //SD card sector read
	output reg[31:0]        sd_sec_read_addr,            //SD card sector read address
	input[7:0]              sd_sec_read_data,            //SD card sector read data
	input                   sd_sec_read_data_valid,      //SD card sector read data valid
	input                   sd_sec_read_end,             //SD card sector read end
    output reg              over,                       //scan complete sign
    output reg[6:0]         amount=7'd0,                 //wav file amount memory
    
    output [31:0]           head_n,
    output [31:0]           length_n,
    output [31:0]           tail_n,
    output [8:0]            minute_now,
    output [5:0]            second_now,
    output [31:0]           aulength_n,
    input  [6:0]            now,                          //wav file current
    input                   enter_n,
    output [31:0]           secon                        //Audio length currently     
);

localparam S_IDLE         = 0;
localparam S_ALIGN      = 1;
localparam S_FIND         = 2;
localparam S_PLAY_WAIT    = 3;
localparam S_PLAY         = 4;
localparam S_END          = 5;

localparam HEADER_SIZE    = 88;

reg[3:0]         state;               //state machine
reg[9:0]         rd_cnt;              //sector read counter
reg[7:0]         header_0;            //first byte of the SD card sector
reg[7:0]         header_1;            //second byte of the SD card sector
reg[7:0]         header_2;            //third byte of the SD card sector
reg[7:0]         header_3;            //fourth byte of the SD card sector

reg[7:0]         header_4;            
reg[7:0]         header_5;            
reg[7:0]         header_6;            
reg[7:0]         header_7;            

reg[31:0]        file_len;            //wav file length
reg[31:0]        audio_len;            //Audio length
wire[31:0]       minute_n;            //Audio minute length currently
wire[31:0]       second_n;            //Audio second length currently
reg [31:0]       head_now_addr;       //head buffer        
wire[31:0]       head_to_tail;        //wav file Byte length         
reg[31:0]        play_cnt;            //play counter
reg                found;               //find a wav file


reg              wea;                         //All RAM input enable
                 

always@(posedge clk or posedge rst)
begin
	if(rst == 1'b1)
		rd_cnt <= 10'd0;
	else if(state == S_FIND)
	begin
		if(sd_sec_read_data_valid == 1'b1)
			rd_cnt <= rd_cnt + 10'd1;
		else if(sd_sec_read_end == 1'b1)
			rd_cnt <= 10'd0;
	end
	else
		rd_cnt <= 10'd0;
end

always@(posedge clk or posedge rst)
begin
	if(rst == 1'b1)
	begin
		header_0 <= 8'd0;
		header_1 <= 8'd0;
		header_2 <= 8'd0;
		header_3 <= 8'd0;
		file_len <= 32'd0;
		found <= 1'b0;
	end
	else if(state == S_FIND && sd_sec_read_data_valid == 1'b1)
	begin
		//file header
		if(rd_cnt == 10'd0)
			header_0 <= sd_sec_read_data;
		if(rd_cnt == 10'd1)
			header_1 <= sd_sec_read_data;
		if(rd_cnt == 10'd2)
			header_2 <= sd_sec_read_data;
		if(rd_cnt == 10'd3)
			header_3 <= sd_sec_read_data;
			
		if(rd_cnt == 10'd8)
			header_4 <= sd_sec_read_data;
		if(rd_cnt == 10'd9)
			header_5 <= sd_sec_read_data;
		if(rd_cnt == 10'd10)
			header_6 <= sd_sec_read_data;
		if(rd_cnt == 10'd11)
			header_7 <= sd_sec_read_data;
			
		//file length
		if(rd_cnt == 10'd4)
			file_len[7:0] <= sd_sec_read_data;
		if(rd_cnt == 10'd5)
			file_len[15:8] <= sd_sec_read_data;
		if(rd_cnt == 10'd6)
			file_len[23:16] <= sd_sec_read_data;
		if(rd_cnt == 10'd7)
			file_len[31:24] <= sd_sec_read_data;
			
	   //Audio length
	   	if(rd_cnt == 10'd74)
		    audio_len[7:0] <= sd_sec_read_data;
		if(rd_cnt == 10'd75)
			audio_len[15:8] <= sd_sec_read_data;
		if(rd_cnt == 10'd76)
			audio_len[23:16] <= sd_sec_read_data;
		if(rd_cnt == 10'd77)
			audio_len[31:24] <= sd_sec_read_data;
			
			
		//wav file header is 'RIFF',in a sector  the first 4 bytes are "RIFF"
		if(rd_cnt == 10'd78 && header_0 == "R" && header_1 == "I" && header_2 == "F" && header_3 == "F" &&
		header_4 == "W" && header_5 == "A" && header_6 == "V" && header_7 == "E")
			found <= 1'b1;
	end
	else if(state != S_FIND)
		found <= 1'b0;
end

//play length counter
always@(posedge clk or posedge rst)
begin
	if(rst == 1'b1)
		play_cnt <= 32'd0;
	else if(state == S_PLAY)
	begin
		if(sd_sec_read_data_valid == 1'b1)
			play_cnt <= play_cnt + 32'd1;
	end
	else if(state == S_END)
		play_cnt <= 32'd0;
end



always@(posedge clk or posedge rst)
begin
	if(rst == 1'b1)
	begin
		state <= S_IDLE;
		sd_sec_read <= 1'b0;
		//search start address
		sd_sec_read_addr <= 32'd8196;
		state_code <= 4'd0;
        amount<=7'd0;
        over<=1'b0;
	end 
    else if(sd_init_done == 1'b0)
	begin
		state <= S_IDLE;
	end
	else
		case(state)
			S_IDLE:
			begin
			 state_code <= 4'd1;
			 wea<=1'b0;
			 if(find == 1'b1&&enter_n==1'b0)
			 begin
              state <= S_ALIGN;
              sd_sec_read_addr<=32'd8196;
              sd_sec_read <= 1'b0;
              amount<=7'd0;
              over<=1'b0;
             end
			end
			
            S_ALIGN:
			begin
		                state <= S_FIND;
			sd_sec_read_addr <= {sd_sec_read_addr[31:3],3'd0};//address 8 aligned
			end

			S_FIND:
			begin
				state_code <= 4'd2;
				if(sd_sec_read_end == 1'b1)
				begin
					if(sd_sec_read_addr>=32'h0008ffff)
                    begin
                        state<=S_IDLE;
                        sd_sec_read <= 1'b0;
                        sd_sec_read_addr<=32'd8196;
		                state_code <= 4'd0;
                        over<=1'b1;
                    end
                   //if you find a wav file
					else if(found == 1'b1)
					begin
						state <= S_PLAY_WAIT;
						head_now_addr<=sd_sec_read_addr;
						sd_sec_read <= 1'b0;
					end
					else
					begin
						//search every 8 sectors(4K)
						sd_sec_read_addr <= sd_sec_read_addr + 32'd8;
					end
				end
				else
				begin
					sd_sec_read <= 1'b1;
				end
			end

			S_PLAY_WAIT:
			begin
		                state <= S_PLAY;
			end

			S_PLAY:
			begin
				state_code <= 4'd3;
                wea<=1'b1;
                sd_sec_read_addr<=head_now_addr+head_to_tail;
                state <= S_END;
			end

			S_END:
			    begin
				state <= S_ALIGN;
				wea<=1'b0;
				amount<=amount+7'd1;
				end
			default:
				state <= S_IDLE;
		endcase
end

ChuFa  ChuF1(
.a(audio_len), 
.b(32'd192000),
.yshang(secon),
.yyushu(),
.Error()
);

ChuFa  ChuF2(
.a(secon), 
.b(32'd60),
.yshang(minute_n),
.yyushu(second_n),
.Error()
);

ChuFa  ChuF3(
.a(file_len), 
.b(32'd512),
.yshang(head_to_tail),
.yyushu(),
.Error()
);

head_memory head_RAM (
  .clka(clk),    // input wire clka
  .wea(wea),      // input wire [0 : 0] wea
  .addra(amount),  // input wire [6 : 0] addra
  .dina(head_now_addr),    // input wire [31 : 0] dina
  .clkb(clk),    // input wire clkb
  .addrb(now),  // input wire [6 : 0] addrb
  .doutb(head_n)  // output wire [31 : 0] doutb
);

length_memory length_RAM (
  .clka(clk),    // input wire clka
  .wea(wea),      // input wire [0 : 0] wea
  .addra(amount),  // input wire [6 : 0] addra
  .dina(file_len),    // input wire [31 : 0] dina
  .clkb(clk),    // input wire clkb
  .addrb(now),  // input wire [6 : 0] addrb
  .doutb(length_n)  // output wire [31 : 0] doutb
);

tail_memory tail_RAM (
  .clka(clk),    // input wire clka
  .wea(wea),      // input wire [0 : 0] wea
  .addra(amount),  // input wire [6 : 0] addra
  .dina(head_now_addr+head_to_tail-32'd1),    // input wire [31 : 0] dina
  .clkb(clk),    // input wire clkb
  .addrb(now),  // input wire [6 : 0] addrb
  .doutb(tail_n)  // output wire [31 : 0] doutb
);

minute_memory minute_RAM (
  .clka(clk),    // input wire clka
  .wea(wea),      // input wire [0 : 0] wea
  .addra(amount),  // input wire [6 : 0] addra
  .dina(minute_n[8:0]),    // input wire [8 : 0] dina
  .clkb(clk),    // input wire clkb
  .addrb(now),  // input wire [6 : 0] addrb
  .doutb(minute_now)  // output wire [8 : 0] doutb
);

second_memory second_RAM (
  .clka(clk),    // input wire clka
  .wea(wea),      // input wire [0 : 0] wea
  .addra(amount),  // input wire [6 : 0] addra
  .dina(second_n[5:0]),    // input wire [5 : 0] dina
  .clkb(clk),    // input wire clkb
  .addrb(now),  // input wire [6 : 0] addrb
  .doutb(second_now)  // output wire [5 : 0] doutb
);

aulength_memory Aulength_RAM (
  .clka(clk),    // input wire clka
  .wea(wea),      // input wire [0 : 0] wea
  .addra(amount),  // input wire [6 : 0] addra
  .dina(audio_len),    // input wire [31 : 0] dina
  .clkb(clk),    // input wire clkb
  .addrb(now),  // input wire [6 : 0] addrb
  .doutb(aulength_n)  // output wire [31 : 0] doutb
);

endmodule
